...
class RequestManager {
    ...
    ExportCookies () {
        const Defaults = {
            ...
            Age:      process.env.DEFAULT_COOKIE_AGE ?? 300,
            Secure:   process.env.DEFAULT_COOKIE_SECURE ?? true,
            HttpOnly: process.env.DEFAULT_COOKIE_HTTP ?? false
        };

        const Jar = this.Cookies.map((Cookie, Key) => [Key, `${Key}=${Cookie.Value}`,
            `Max-Age=${Cookie.hasOwnProperty("Max-Age") ? Cookie["Max-Age"] : Defaults.Age}`,
            "SameSite=Strict", `Domain=${Cookie.Domain ?? Defaults.Domain}`, "Path=/"
        ]).map(Output => {
            const Cookie = this.Cookies.get(Output.shift());
            if (Cookie.HttpOnly ?? Defaults.HttpOnly) Output.push("HttpOnly");
            if (Cookie.Secure ?? Defaults.Secure) Output.push("Secure");
            return Output.join("; ");
        });
