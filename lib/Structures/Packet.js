...
class Packet {
     ...
     * @param {Number} [Age] If creating a cookie, an age in seconds. Set to -1 for a session cookie.
     * @returns {Object|DataStore}
     */
    Cookie (Key, Value, Age) {
        // Set
        if (Key && Value) {
            const Meta = {Value: decodeURIComponent(Value)};
            if (typeof Age === "number") Meta["Max-Age"] = Age;
            this.Request.Cookies.set(Key, Meta);
